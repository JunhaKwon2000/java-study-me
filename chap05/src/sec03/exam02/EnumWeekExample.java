...
public class EnumWeekExample {
	...
	public static void main(String[] args) {
		...
		Week today = null; // 열거 타입 변수 선언(참조 타입이라 null 로 초기화 가능)
		
		Calendar cal = Calendar.getInstance(); // Calendar 객체 열기
		int day = cal.get(Calendar.DAY_OF_WEEK); // 오늘의 요일 얻기(1: 일요일 ~ 7: 토요일)
		...
		System.out.println("오늘의 요일 : " + today); // 열거 객체의 문자열이 출력(열거 상수의 이름과 똑같은 문자열을 갖는다)
		
		// 열거값 비교
		if (today == Week.SUNDAY) System.out.println("일요일에는 배드민턴을 칩니다.");
		else System.out.println("열심히 자바 공부합니다.");
		// 숫자 1, 2, 3 으로 비교하는 것보다 훨씬 가독성이 좋음
